...
import fieldMapping, { AI_FIELD_KEYS } from '../../config/field-mapping.js';
...
const buildPrompt = (issue) => {
  ...
  const fieldList = AI_FIELD_KEYS.join(', ');
...
const analyzeWithOpenAI = async (issue) => {
...
export const analyzeIssue = async (issue) => {
  if (!isOpenAIEnabled()) {
    return analyzeIssueRuleBased(issue);
  }

  return analyzeWithOpenAI(issue);
};
